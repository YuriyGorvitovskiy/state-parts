...
export type FieldComparison = 'any' | '=' | '!=' | 'in' | 'not in' | '><' | '<>' | '<=' | '<' | '>' | '>=' | 'like' | 'not like';
...
export declare type SubQuery<R> = {
    readonly [key in keyof R | '$type' | '$rev' | '$cmp' | '$field' | '$select']?
    : key extends '$type' ? string
    : key extends '$rev' ? boolean
    : key extends '$cmp' ? ChildComparison
    : key extends '$field' ? string
    : key extends '$select' ? boolean
    : key extends keyof R ? SubQuery<R[key]> | Field | value
    : void;
};
...
export declare type StrictSubQuery<R> = {
    ...
    : key extends '$field' ? string
    : key extends '$select' ? boolean
    : key extends keyof R ? StrictSubQuery<R[key]> | StrictField
    : void;
};

export declare type Query<R> = {
    readonly [key in keyof R | '$type']?
    : key extends '$type' ? string
    : key extends keyof R ? SubQuery<R[key]> | Field | value
    : void;
};
...
>(k: keyof R, v: F, parentSelect: boolean): O => {
    return isValue(v)
