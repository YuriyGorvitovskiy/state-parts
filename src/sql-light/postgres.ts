...
const literalToSql = (f: SQL.Literal): string => {
    ...
    switch (typeof f.val) {
        ...
        case 'string': return "E" + escape(val);
        default: break;
    }
...
const betweenToSql = (inv: boolean, args: List<SQL.Expression>): string => {
    return expressionToSql(args.get(0), true)
...
const inToSql = (inv: boolean, args: List<SQL.Expression>): string => {
    return expressionToSql(args.get(0), true)
        + (inv ? " NOT IN (" : " IN (")
        + args.shift().map(a => expressionToSql(a, false)).join(", ")
        + ")";
}

const isNullToSql = (inv: boolean, args: List<SQL.Expression>): string => {
    return expressionToSql(args.get(0), true)
        + (inv ? " IS NOT NULL" : " IS NULL");
}

const predicateToSql = (p: SQL.Predicate): string => {
    switch (p.cmp) {
        case 'and': return booleanToSql(p.inv, "AND", p.args);
        case 'or': return booleanToSql(p.inv, "OR", p.args);
        case 'between': return betweenToSql(p.inv, p.args);
        case 'equal': return operatorToSql(p.inv ? "!=" : "=", p.args);
        case 'greater': return operatorToSql(p.inv ? "<=" : ">", p.args);
        case 'lesser': return operatorToSql(p.inv ? ">=" : "<", p.args);
        case 'like': return operatorToSql(p.inv ? "NOT LIKE" : "LIKE", p.args);
        case 'in': return inToSql(p.inv, p.args);
        case 'is null': return isNullToSql(p.inv, p.args);
        default: break;
    }
...
export const toSql = (select: SQL.Select): string => {
    return "SELECT " +
        ...
        select.joins.map(j => joinToSql(j)).join("\n") +
